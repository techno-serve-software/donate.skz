        ...
        <table class="table table-stripedd">
            ...
            <tbody>
                ...
                <tr data-row-id="{{ $key + 1 }}">
                    ...
                    @if ($participant_names)
                    @if ($cart->category_name=='Qurbani')
                    <td class="form-cart-item-quantity" data-toggle="tooltip" data-placement="left"
                        title="{{ $cart->category_name=='Qurbani'?'To add more Qurbani(s), Please add from the above – Participant Name':''}}">
                        <div class="input-group
                                            input-group-spinner">
                            <span class="input-group-btn">
                                <button class="btn
                                                    btn-default btn-minus" type="button"
                                    {{ $cart->category_name=='Qurbani'?'disabled':''}}><i class="fa
                                                        fa-minus"></i></button>
                            </span>
                            <input type="text" class="form-control" value="{{ $cart->quantity }}"
                                {{ $cart->category_name=='Qurbani'?'disabled':''}}>
                            <span class="input-group-btn">
                                <button class="btn btn-default btn-plus" type="button"
                                    {{ $cart->category_name=='Qurbani'?'disabled':''}}><i
    ...
    <div class="row">
        ...
        <div class="col col-lg-8 col-md-8">
            ...
            <a href="#donate-form" class="btn btn-md
                                btn-default
                                mb-sm-10">Add more donations</a>
            <button type="submit" class="btn btn-md
                                btn-primary">Proceed to Payment</button>
        </div>
    </div>
</form>
@else
<div class="text-center">
    <p class="font-15"> Nothing here. <a href="#donate-form">Add Qurbani</a>
    </p>
</div>
@endif
